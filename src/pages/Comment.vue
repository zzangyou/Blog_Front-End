<template>
  im查看已发布评论
  <div class="showmycomment">
    <div>
      {{ totalnumber }}
      <div v-for="(b, index) in blogs" :key="index" class="myblog">{{ b.like }}</div>
    </div>
...
export default {
  setup() {
    ...
    onMounted(() => {
      const userComment = proxy.$api.getUserComment({ useraccount: localStorage.getItem('currentuser') });
      console.log('查看已发布评论', userComment);
      userComment.then(
        (value) => {
          console.log(value);
